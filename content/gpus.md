...
本文分为三个篇章，分别是**个人消费级**、**发烧友级**和**企业级**，请根据你的需求往下翻。
...
我建议技术发烧友的首选考虑是**2080TI 22G版本**，然后是**3080 20G版本**，以及**3090 24G版本**和 **4090D/4090 24G版本**。
...
![alt text](/images/GPU推荐/ausu_b660_plus.png)
...
1. **B760M-PLUS**（Intel 13/14代） + **2** x **4060ti 16G** 价格约 1w2
2. **B760M-PLUS**（Intel 13/14代） + **1** x **3090 24G** 价格约 1w2
3. **B760M-PLUS**（Intel 13/14代） + **1** x **4090 24G** 价格约 2w
...
![alt text](/images/GPU推荐/supermicro-7048.jpg)
...
这些服务器主板支持IPMI，可实现远程开关和操控服务器。2080Ti使用PCIe3.0接口，因此选择支持PCIe3.0的主板即可。根据推理模型的内存需求，推荐使用128GB内存。
购买主机时需注意确保配件和设置的兼容性，例如显卡电源线、服务器输出、IPMI密码等，避免购买问题或不兼容的设备。
...
因此，我们推荐企业采用**Int8及以上的模型**来运行实际业务。这种模式的性能损失可以控制在1%以内，显存占用却可以缩减一半（1B~1G）。
...
这里给大家推荐一款华硕的服务器主机**华硕ECS8000 G4** ，单主机价格约2.5万元。具体参数看这里：https://servers.asus.com.cn/products/servers/gpu-servers/ESC8000-G4 。
...
![alt text](/images/GPU推荐/Dell-XE9680.jpg)

从这里开始，我们终于可以讨论**真正的企业级运算设备**！
...
冗余 3000W Titanium Power Supply
支持高性能计算、AI/深度学习训练、工业自动化等应用
...
![alt text](/images/GPU推荐/msi_nvlink.jpg)
